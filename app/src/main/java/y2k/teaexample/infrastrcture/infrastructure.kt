...
object RecycleViewModule {
    ...
    class MyRecycleView_<T> : RecyclerView_() {
        ...
        override fun createEmpty(context: Context) = MyRecycleView<Any>(context)

        @Suppress("UNCHECKED_CAST")
        override fun update(p: Property, a: View) {
            val view = a as MyRecycleView<T>
            when (p.propId) {
                3000 -> view.items = p.value as List<T>
                3001 -> view.idMapping = p.value as (T) -> Any?
                3002 -> view.mapping = p.value as (T) -> VirtualNode
            }
        }
    }

    class MyRecycleView<T> @JvmOverloads constructor(
        context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0
    ) : RecyclerView(context, attrs, defStyleAttr) {

        private val adapter = object : ListAdapter<T, ViewHolder>(
            object : DiffUtil.ItemCallback<T>() {
                override fun areItemsTheSame(oldItem: T, newItem: T) = idMapping(oldItem) == idMapping(newItem)
        ...
        var idMapping: (T) -> Any? by Delegates.observable({ x -> x }) { _, _, _ ->
            adapter.notifyDataSetChanged()
        }

        var mapping: (T) -> VirtualNode by Delegates.observable({ _ -> TODO() }) { _, _, _ ->
            adapter.notifyDataSetChanged()
        }
